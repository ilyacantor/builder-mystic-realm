...
const gear = [
  {
    ...
    description: "Unmatched power transfer and comfort for long-distance rides with responsive carbon soles.",
    label: "Read Review",
  },
  {
    title: "Gravel Helmet 'Trailblazer'",
    description: "Designed for speed, offering superior protection and aerodynamic ventilation.",
    label: "Read Review",
  },
];
      ...
      <main className="mx-auto flex max-w-6xl flex-col gap-12 px-6 py-12">
                ...
                {['Road', 'Gravel', 'Triathlete'].map((option) => (
                  <button
                    ...
                    className={`rounded-full px-5 py-2 text-xs font-semibold uppercase tracking-[0.3em] ${
                      option === 'Road' ? 'bg-[#00bcc4] text-black' : 'border border-white/10 bg-black/40 text-white/70'
                    }`}
                  >
                    {option}
                  </button>
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {localEvents.map((event) => (
              <article key={event.title} className="flex flex-col rounded-[1.75rem] border border-white/10 bg-[#10131d]">
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_1fr]">
          <article className="rounded-[2rem] border border-white/10 bg-[#10131d] p-8">
            <h2 className="text-base font-semibold text-white">Connect with Your Tribe</h2>
            <div className="mt-6 space-y-4">
              {tribe.map((member) => (
                <div key={member.name} className="flex items-center justify-between rounded-2xl border border-white/10 bg-[#0f121b] px-4 py-3">
                  <div>
        ...
        <section className="rounded-[2rem] border border-white/10 bg-[#10131d] p-8">
          <h2 className="text-base font-semibold text-white">Training Integrations</h2>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {integrations.map((integration) => (
              <div key={integration.name} className="rounded-2xl border border-white/10 bg-[#0f121b] px-4 py-4 text-center">
                ...
                <span
                  className={`mt-3 inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] ${
                    integration.status === 'Disconnected' ? 'bg-[#ff5858] text-white' : 'bg-[#00bcc4] text-black'
                  }`}
                >
                  {integration.status}
                </span>
        ...
        <section className="rounded-[2.5rem] border border-white/10 bg-gradient-to-r from-[#00bcc4] via-[#00cbe0] to-[#6f89ff] px-10 py-12 text-center text-[#0b0f17]">
          <h2 className="text-3xl font-semibold">Join the FlowNation Community Today!</h2>
